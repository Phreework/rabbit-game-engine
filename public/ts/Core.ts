...
export class Rect extends RabObject {
    ...
    collideRect(rect) {
        if (this.x > rect.x + rect.w / 2 + this.w / 2)
            return false;
        if (rect.x > this.x + this.w / 2 + rect.w / 2)
            return false;
        if (this.y > rect.y + rect.h / 2 + this.h / 2)
            return false;
        if (rect.y > this.y + this.h / 2 + rect.h / 2)
            return false;
        return true;
    }
