...
export default class SuperGame {
    ...
    private BulletShoot() {
        const tween = new Tween(this.player).to({},0.3).onComplete(() => {
            console.log("执行拉拉"); this.shootOneBullet();
             tween.start(); 
        }).start();
    }
    private gameOver() {
    ...
    private initPlayer() {
        ...
        playerTouchCtl.startEvent = () => {
            (player as any).startPoint = new Vec3(player.transform.position);
        };
...
export class TouchController {
    ...
    destroy() {
        ...
        this.target.listenOff(Entity.EventType.MOUSE_PRESS);
        // this.target.listenOff("touchcancel");
    }
}
